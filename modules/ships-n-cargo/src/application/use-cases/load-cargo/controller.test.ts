...
test('cannot load the same cargo (name as identifier) twice', () => {
  ...
  const response3 = loadCargoController.loadCargo(request)
...
test('create throws an unexpected application error', () => {
  ...
  const response: Response = loadCargoController.loadCargo(request)
