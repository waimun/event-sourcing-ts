...
import { GetShipHistoryQuery } from './query'
...
test('returns projected ship history', async () => {
  ...
  await expect(new GetShipHistoryQuery(projection).execute(new Id('ship-1'))).resolves.toEqual({
    ok: true,
    value: history
  })
})
...
test('returns ship not found when no history has been projected', async () => {
  ...
  await expect(new GetShipHistoryQuery(projection).execute(new Id('missing'))).resolves.toEqual({
    ok: false,
