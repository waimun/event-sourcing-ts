...
test('depart command with a Ship object that is uninitialized', () => {
  expect(() => {
    Ship.depart(new SailShip('123'), Ship.uninitialized())
...
test('depart command with a Ship object that has a different aggregate ID', () => {
  ...
  expect(() => {
    Ship.depart(new SailShip('456'), ship)
  }).toThrow(IdsMismatch)
})
...
test('depart command with a missing port', () => {
  ...
  expect(() => {
    Ship.depart(new SailShip('123'), ship)
...
test('depart command with a Ship that is at sea (already departed)', () => {
  ...
  expect(() => {
    Ship.depart(new SailShip('123'), ship3)
  }).toThrow(InvalidPortForDeparture)
})
...
test('load cargo', () => {
  ...
  const event = Ship.loadCargo(new LoadCargo('123', new Cargo('Refactoring Book')), ship)
...
test('load cargo requires a Ship be created first', () => {
  expect(() => {
    Ship.loadCargo(new LoadCargo('123', new Cargo('Refactoring Book')), Ship.uninitialized())
  }).toThrow(ShipMustBeCreatedFirst)
})
...
test('load cargo command with a Ship object that has a different aggregate ID', () => {
  ...
  expect(() => {
    Ship.loadCargo(new LoadCargo('456', new Cargo('Refactoring Book')), ship)
  }).toThrow(IdsMismatch)
})

test('load cargo with a cargo that is already loaded', () => {
  const ship = Ship.apply(Ship.uninitialized(), new ShipCreated('123', 'King Roy'))
  const cargoLoaded = Ship.loadCargo(new LoadCargo('123', new Cargo('Refactoring Book')), ship)
  const ship2 = Ship.apply(ship, cargoLoaded)

  expect(() => {
    Ship.loadCargo(new LoadCargo('123', new Cargo('Refactoring Book')), ship2)
...
test('unload cargo', () => {
  ...
  const cargoUnloaded = Ship.unloadCargo(new UnloadCargo('123', new Cargo('Refactoring Book')), ship2)
...
test('unload cargo requires a Ship be created first', () => {
  expect(() => {
    Ship.unloadCargo(new UnloadCargo('123', new Cargo('Refactoring Book')), Ship.uninitialized())
  }).toThrow(ShipMustBeCreatedFirst)
})
...
test('unload cargo with a Ship object that has a different aggregate ID', () => {
  ...
  expect(() => {
    Ship.unloadCargo(new UnloadCargo('456', new Cargo('Refactoring Book')), ship)
  }).toThrow(IdsMismatch)
})
...
test('unload cargo with a cargo that is not found on the Ship', () => {
  ...
  expect(() => {
    Ship.unloadCargo(new UnloadCargo('123', new Cargo('Refactoring Book')), ship)
