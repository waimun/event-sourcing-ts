...
test('container id is required', async () => {
  req.body = { id: 'abc', description: 'Microservices Architecture' }
...
test('container id is invalid', async () => {
  req.body = { id: 'abc', containerId: 'a#*x', description: 'Microservices Architecture' }
...
test('container description is invalid', async () => {
  ...
  expect(res.json).toHaveBeenCalledWith({
    status: 400,
    error: new NameNotAllowed(req.body.description, 'Container description').message,
    dateTime: expect.any(Date)
  })
})
...
test('cannot load same container twice', async () => {
  ...
  // cannot load same container twice
  await loadContainer(req as Request, res as Response)
  expect(res.status).toHaveBeenNthCalledWith(3, 409)
  expect(res.json).toHaveBeenNthCalledWith(3, {
    status: 409,
    error: new ContainerAlreadyLoaded(req.body.containerId).message,
    dateTime: expect.any(Date)
  })
})
